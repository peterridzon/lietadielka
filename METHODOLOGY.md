...
The purpose of a flight is **never** inferred automatically. It is entered by hand with
`npm run purpose:set`, which refuses any status other than `unknown` without a publisher
and a source URL. No political conclusion is generated by this software.

The three statuses mean different things, and the difference is the whole point:

* **confirmed** — a source states that this aircraft carried this mission.
* **probable** — the evidence is circumstantial. Typically an officially announced trip
  whose dates and destination match a detected flight, with nothing tying the delegation
  to the airframe. Matching dates are a coincidence until someone says otherwise.
* **unknown** — the default, and where most flights stay.

Worked example, recorded in the database: OM-BYA sat on the ground at Newark Liberty for
76 hours from 12 to 15 July 2026. The Ministry of Foreign and European Affairs announced a
work trip by the foreign minister to the United States from 12 to 16 July 2026 to present
Slovakia's UN Security Council candidacy. The dates line up exactly — and the flight is
still recorded as `probable`, because no source says the delegation flew on that aircraft.
