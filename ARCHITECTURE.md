...
| ----------- | --------------------- | -------------------- | ----------- |
| `adsblol`   | daily trace archive   | **~40–45 days rolling** | none     |
...
> opposite things for data coverage, so the adapter disambiguates with a *day sentinel*:
> a handful of airliners that fly almost every day. A sentinel trace for the day proves
> the archive holds the day, so a `504` for our aircraft really does mean it was not
> seen. No sentinel either, and the day is reported as unavailable. When in doubt the
> answer is "unavailable" — undercounting coverage is safe, undercounting flights is not.
